...
class SpidersPipeline:
    def insert_one_to_one(self, items):
        PropertyData.objects.bulk_create(
            [PropertyData(
                agent = EstateAgent.objects.get(**one_to_one["foreign_key"]),
                **one_to_one["normal"],
            )for one_to_one in items],
            ignore_conflicts=True
        )
    
    def insert_one_to_many(self, items, model: models.Model):
        model.objects.bulk_create(
            [model(
                property_id = Property.objects.get(**item["id"]),
                **item["data"]
            )for item in items],
            ignore_conflicts=True
        )

    def insert_agents(self, agents, accreditations, join):
        ...
        AgentAccreditation.objects.bulk_create(
            ...
            )for agentaccreditation in join],
            ignore_conflicts=True
        )

    def update_property_unpublished(self, items):
        properties = Property.objects.filter(property_id__in=items)
        for idx, _ in enumerate(properties):
            properties[idx].published = False
    ...
    def update_property_scraped(self, items): 
        properties = []
        for id in items: 
            property = Property.objects.get(property_id = id)
            property.property_data = PropertyData.objects.get(property_id = id)
    ...
    def process_item(self, item, spider):
        ...
        for property in item:
            property_section: dict = property["props"]["pageProps"].get("property", {}) or {}
            property_id = property_section.get("pathId")

            if not property["props"]["pageProps"]["published"]:
                unpublished.append(property_id)
            ...
            style_section: dict = property_section.get("style", {}) or {}
        ...
        self.insert_agents(agents, accreditations, agentaccreditations)
        self.insert_one_to_many(items=listing_histories, model=ListingHistory)
        self.insert_one_to_many(items=views, model=Views)
        self.insert_one_to_many(items=key_features, model=KeyFeature)
        self.insert_one_to_one(items=one_to_ones)
        self.update_property_unpublished(items=unpublished)
        self.update_property_scraped(items=property_ids)
